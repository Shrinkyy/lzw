...
/**
 * @Author 娄志伟
 * @Create 2023/6/11 16:40
 */
public interface UserDao {
    /** 插入一条分类信息
     * @param user 包含了要添加的分类信息 其中id为空 因为id是自增长的 不需要开发者提供
     */
    void insert(User user);

    /**  根据id修改分类的name属性值
     * @param user 提供需要被修改的name字段的值 已经根据哪一个id进行update的主键id
     *  update category set name = ? where id = ?
     */
    void update(User user);

    /** 根据id或者name进行删除
     * @param user 如果提供的id不为空 则根据id删除 如果提供的name不为空 且id为空
